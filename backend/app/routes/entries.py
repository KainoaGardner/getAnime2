...
from app.database.schemas import Entry, EntryIds
...
def add_entries(
    auth: user_dependency, anime_ids: EntryIds, db: Session = Depends(get_db)
):
    user_id = auth["id"]

    return entries.add_anime(db, user_id, anime_ids.entries)
...
def delete_entries(
    ...
    return entries.remove_anime(db, user_id, anime_ids.entries)
